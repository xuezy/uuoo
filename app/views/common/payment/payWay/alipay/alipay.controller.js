(function() {
	'use strict';
	//支付宝
	angular.module('zhyyt.alipay', [])
		.controller('alipayController', function($scope, $rootScope, $state, $interval, $timeout, commonService, barcodeService, rechargeService, payMianService) {
			var orderInfo = commonService.getState();
			var chargeOrderNo; //支付充值专用订单号
			$rootScope.headerTitle = '支付宝'; //头部显示名字
			$scope.open = false;
			$scope.payState = '';
			$scope.AuthCode = '';
			$scope.nORb = true;
			...
			function query(orderNumber) {
				...
				var params = {
					...
					OrderNo: orderNumber,
					OrderDate: orderNumber.substr(4, 8)
				};
				if (orderInfo.BusiCode === '01') {
