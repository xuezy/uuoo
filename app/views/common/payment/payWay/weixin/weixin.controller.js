(function() {
	'use strict';
	//微信
	angular.module('zhyyt.weixin', [])
		.controller('weixinController', function($scope, $rootScope, $state, $interval, $timeout, commonService, barcodeService, rechargeService, payMianService) {
			var orderInfo = commonService.getState();
			var chargeOrderNo; //支付充值专用订单号
			$rootScope.headerTitle = '微信支付'; //头部显示名字
			$scope.open = false; //取消支付模态框开关
			$scope.payState = '';//加载页面模态框开关
			$scope.AuthCode = '';//18位条形码
			$scope.nORb = true; //条形or二维码
			...
			//生成二维码
			var nativeNum = 0;//如果是充值缴费，只能生成一次二维码
			$scope.native = function() {
				if (orderInfo.BusiCode === '01') {//纯支付
					var param = {
				...
				} else if (orderInfo.BusiCode === '02') {//充值缴费
					if (nativeNum > 0) {
						$timeout(function() {
